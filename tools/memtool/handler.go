...
type MemtoolInput struct {
	...
	PID           int    `json:"pid,omitempty" jsonschema:"Target process ID (for regions, search, read, write, disasm, struct_search, pointer_scan, diff)"`
	...
	Length        int    `json:"length,omitempty" jsonschema:"Bytes to read/disasm (default 256 for read, 64 for disasm, max 4096)"`
	Protection    string `json:"protection,omitempty" jsonschema:"Filter by protection: r, rw, rx"`
	MaxResults    int    `json:"max_results,omitempty" jsonschema:"Max results to display (default 100, max 1000)"`
	...
	MaxDepth      int    `json:"max_depth,omitempty" jsonschema:"Pointer scan max chain depth (default 3, max 5)"`
	MaxOffset     int    `json:"max_offset,omitempty" jsonschema:"Pointer scan max offset from target (default 0x1000)"`
	Arch          string `json:"arch,omitempty" jsonschema:"CPU architecture for disasm: x86 (default) or arm"`
	Mode          int    `json:"mode,omitempty" jsonschema:"CPU mode for disasm: 32 or 64 (default)"`
	Count         int    `json:"count,omitempty" jsonschema:"Number of instructions to disassemble (default 50, max 200)"`
}
...
func opRegions(input MemtoolInput) (*mcp.CallToolResult, MemtoolOutput, error) {
	...
	if err := reader.Open(input.PID, false); err != nil {
	...
	return successResult(formatRegions(regions, input.MaxResults))
}
...
func opWrite(input MemtoolInput) (*mcp.CallToolResult, MemtoolOutput, error) {
	...
	reader := newProcessReader()
	if err := reader.Open(input.PID, true); err != nil {
		return errorResult("%v", err)
	}
	defer reader.Close()
...
func opInfo(input MemtoolInput) (*mcp.CallToolResult, MemtoolOutput, error) {
	...
	return successResult(formatSessionInfo(session, session.reader, input.MaxResults))
}
...
func opStructSearch(input MemtoolInput) (*mcp.CallToolResult, MemtoolOutput, error) {
	...
	return successResult(formatStructSearchResult(addresses, input.MaxResults))
}
...
func opDiff(input MemtoolInput) (*mcp.CallToolResult, MemtoolOutput, error) {
	if input.PID <= 0 && input.SessionID == "" {
	...
	if input.SessionID != "" {
		...
		diffs, err := compareWithSnapshot(session.reader, session.snapshot, input.MaxResults)
		if err != nil {
		...
		return successResult(formatDiffResult(diffs, input.MaxResults))
	}
