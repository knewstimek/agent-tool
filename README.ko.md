...
- **도구가 너무 분산됨**: 에이전트에게 Redis CLI, MySQL 클라이언트, SSH 클라이언트 등을 찾고, 설치하고, 설정하게 시키는 과정 자체가 번거롭고 오류가 잦음. agent-tool은 45개 도구를 단일 바이너리로 통합 — 한 번 설치하면 전부 동작.
...
| **Analyze** | 정적 바이너리 분석 — x86/x64 디스어셈블리, PE 헤더 파싱, 문자열 추출, hexdump | ✅ |
