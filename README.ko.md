...
| **Analyze** | 정적 바이너리 분석 및 리버스 엔지니어링. x86/x64/ARM/ARM64 디스어셈블리 (PE VA 파라미터 지원 -- 자동 VA 표시, 심볼 주석, 함수 경계 자동 중지). PE/ELF/Mach-O 파싱 (RWX 경고, 리소스, IAT VA 포함 임포트, 익스포트). xref (특정 주소에 대한 코드 참조 검색). function_at (.pdata 또는 휴리스틱 프롤로그/에필로그 스캔으로 함수 경계 탐지). call_graph (루트 함수 기준 정적 콜 그래프 -- caller/callee 트리 및 IAT import 탐지). imphash, Rich 헤더, DWARF 디버그 정보, 문자열 추출(VA 포함), hexdump, 헥스 패턴 검색(VA 포함), 엔트로피 분석, 오버레이 탐지, 바이너리 비교 | ✅ |
