...
| **Analyze** | 정적 바이너리 분석 및 리버스 엔지니어링. x86/x64/ARM/ARM64 디스어셈블리 (PE VA 파라미터 지원 -- 자동 VA 표시, 심볼 주석, 함수 경계 자동 중지, stop_at_ret). PE/ELF/Mach-O 파싱 (RWX 경고, 리소스, IAT VA 포함 임포트, 익스포트). xref (요약 통계 포함). function_at (.pdata 또는 휴리스틱 프롤로그/에필로그 스캔으로 함수 경계 탐지). call_graph (정적 콜 그래프). follow_ptr (포인터 체인 추적, 순환 참조 감지). rtti_dump (MSVC RTTI 파싱, 클래스명 디맹글링, pSelf 검증). vtable_scan (.rdata에서 RTTI가 있는 vtable 자동 발견). struct_layout (메모리 레이아웃 어노테이션). imphash, Rich 헤더, DWARF 디버그 정보, 문자열 추출(VA 포함), hexdump, 헥스 패턴 검색(섹션명 표시), 엔트로피 분석, 오버레이 탐지, 바이너리 비교 | ✅ |
