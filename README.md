...
| **Analyze** | Static binary analysis and reverse engineering. x86/x64/ARM/ARM64 disassembly with PE VA parameter support (auto VA display, symbol annotation, function boundary auto-stop). PE/ELF/Mach-O parsing (RWX warnings, resources, imports with IAT VA, exports). xref (find code references to any address). function_at (find function boundaries via .pdata or heuristic prologue/epilogue scan). call_graph (static call graph from a root function with caller/callee tree and IAT import detection). imphash, Rich header, DWARF debug info, string extraction (with VA), hexdump, hex pattern search (with VA), entropy analysis, overlay detection, binary diff | ✅ |
...
| **Wintool** | Windows GUI automation — find/enumerate windows and child controls, capture screenshots (base64 PNG via PrintWindow), read/set text, click, type, send raw messages, show/hide/minimize/maximize, move/resize, close, focus. Enables AI agents to "see" and interact with GUI applications. Windows only. Note: apps with DRM/overlay protection (some games, streaming apps) may return black screenshots — this is a Windows-level capture restriction, not a tool limitation | ✅ |
